    ...
    series: [
      ...
      },
      {
        ...
        emphasis: { 
          itemStyle: { 
            shadowBlur: 15, 
            shadowColor: 'rgba(205,92,92,0.6)',
            shadowOffsetY: 5
          } 
        },
        ...
        >
          {dynastyFeatures.map((d, i) => (
            <div key={i} className="card-glass p-4 text-center group cursor-default">
              ...
              <div className="text-xs text-amber-300 font-medium">{d.style}</div>
              <div className="mt-2.5 flex flex-wrap gap-1 justify-center">
                {d.features.map((f, j) => (
                  <span key={j} className="text-[9px] px-1.5 py-0.5 bg-amber-900/20 text-amber-400 rounded-full border border-amber-800/15">
                    {f}
                  </span>
                ))}
              </div>
