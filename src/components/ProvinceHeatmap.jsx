    ...
    dataZoom: [
      {
        ...
        fillerColor: 'rgba(197,165,90,0.15)',
        handleStyle: { 
          color: '#C5A55A',
          shadowBlur: 10,
        ...
        data: sorted.map((d, i) => ({
          ...
          itemStyle: {
            color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
              { offset: 0, color: i < 5 ? '#CD5C5C' : i < 10 ? '#DAA520' : '#B0C4DE' },
              { offset: 1, color: i < 5 ? '#BC8F8F' : i < 10 ? '#B8860B' : '#708090' },
            ]),
        ...
        >
          <div className="flex items-center gap-6 mb-4 text-xs text-gray-500">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm" style={{ background: '#CD5C5C' }} />
              TOP 5
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm" style={{ background: '#DAA520' }} />
              TOP 6-10
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm" style={{ background: '#B0C4DE' }} />
              其他省份
            </span>
          </div>
          <ReactEChartsCore
