  ...
  useEffect(() => {
    ...
    const option = {
      ...
      tooltip: {
        ...
        borderWidth: 1,
        textStyle: { color: '#e8e0d0', fontSize: 13, fontFamily: 'Noto Sans SC' },
        formatter: (p) => {
          if (p.dataType === 'edge') {
            return `<div style="font-size:14px;font-weight:bold;color:#C5A55A;margin-bottom:4px">${p.data.source} → ${p.data.target}</div>
                   <div style="color:#FFD700">${p.data.value}</div>`
          }
          return `<div style="font-size:14px;font-weight:bold;color:#C5A55A">${p.name}</div>`
        },
      },
      ...
      series: [{
        ...
        roam: true,
        draggable: true,
        force: { repulsion: 300, gravity: 0.08, edgeLength: [100, 200], friction: 0.6 },
        label: {
          show: true, 
          position: 'right', 
          color: '#e8e0d0', 
        ...
        emphasis: {
          ...
          lineStyle: { 
            ...
            opacity: 0.9,
            shadowBlur: 10,
            shadowColor: 'rgba(197,165,90,0.6)'
          },
          itemStyle: { 
            shadowBlur: 25, 
            shadowColor: 'rgba(197,165,90,0.5)',
            borderColor: '#fff',
            borderWidth: 2
          },
          label: {
        ...
        symbolSize: (val, params) => {
          return params.data.symbolSize * 1.2;
        },
      }],
    }
