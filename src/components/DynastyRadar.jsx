...
const dynastyColors = {
  '秦': '#708090', '汉': '#CD5C5C', '唐': '#BC8F8F',
  '宋': '#B0C4DE', '元': '#90EE90', '明': '#F08080', '清': '#DAA520',
}
...
export default function DynastyRadar() {
  ...
  const getOption = () => ({
    tooltip: {
      ...
      formatter: (params) => {
        let html = `<div style="font-size:16px;font-weight:bold;color:#C5A55A;margin-bottom:6px">${params.name}</div>`
        html += `<div style="margin-top:4px">`
        params.value.forEach((val, i) => {
          const indicator = ['建筑规模', '工程技术', '艺术成就', '历史影响', '保存程度'][i]
    ...
    radar: {
      ...
      splitArea: {
        areaStyle: {
          color: ['rgba(197,165,90,0.02)', 'rgba(197,165,90,0.05)', 'rgba(197,165,90,0.08)'],
        }
      },
      axisLine: { 
        lineStyle: { 
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-bold transition-all duration-300 border ${
                selected.includes(d.dynasty)
                  ...
                  : 'bg-surface text-ink border-gray-300/30 hover:border-gray-500/50'
              }`}
